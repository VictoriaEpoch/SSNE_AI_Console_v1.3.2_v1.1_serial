...
def build_zone_set(points: Iterable[tuple[float, float]]) -> str:
    ...
    values = " ".join(f"{value:.4g}" for point in normalized for value in point)
...
def build_zone_rect(x1: float, y1: float, x2: float, y2: float) -> str:
    ...
    values = " ".join(f"{value:.4g}" for value in (*first, *second))
...
def build_zone_upload(points: Iterable[tuple[float, float]]) -> list[str]:
    ...
    commands.extend(validate_command(f"zone add {x:.4f} {y:.4f}") for x, y in normalized)
...
def build_enroll(person_id: str, display_name: str, frames: int = 15) -> str:
    ...
    return validate_command(f"reg {person_id} {display_name} {frames}")
...
def build_print_interval(value: int) -> str:
    ...
    return validate_command(f"set print_interval {value}")
